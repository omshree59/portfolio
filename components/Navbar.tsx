...
export default function Navbar() {
  return (
    <motion.nav
      ...
      className="fixed top-4 left-4 right-4 md:top-6 md:left-12 md:right-12 z-[999] flex flex-col items-end gap-2"
    >
      {/* Brand Icon / Logo */}
      ...
      {/* Navigation Links */}
      <div className="flex items-center gap-1 p-1.5 rounded-full bg-black/70 backdrop-blur-2xl border border-white/20 shadow-[0_10px_40px_rgba(0,0,0,0.8)]">
        {navItems.map((item) => (
          <a
            key={item.name}
            href={item.href}
            // 🔥 FIX: Made the default text color brighter and font-bold
            className="px-4 md:px-5 py-2.5 rounded-full text-sm font-bold text-zinc-200 hover:text-white hover:bg-white/15 focus-visible:bg-white/20 focus-visible:text-white transition-all duration-300"
          >
            {item.name}
          </a>
        ))}
