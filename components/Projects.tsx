...
const projects = [
  ...
  {
    title: "LexiCue - Sign Language - Text & Speech",
    description: "LexiCue is a Live sign language interpretation using computer vision and deep learning.",
    tech: ["React","TypeScript","FastAPI", "Python","MediaPipe","PyTorch","WebSocket","Recharts"],
    githubLink: "https://github.com/omshree59/SIGN-Language-Detector-HandSpeak-AI",
    liveLink: "",
    image: "/sign-lang-detector.png",
...
const SpotlightCard = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 transition duration-300 z-0"
        style={{
          opacity,
...
export default function Projects() {
  ...
  const RED_DURATION = 2200;    
  const YELLOW_DURATION = 3500; 
  const GREEN_DURATION = 1300;  
        ...
        <AnimatePresence initial={false} custom={direction} mode="popLayout">
          {showCard && (
            ...
            <SpotlightCard className="relative rounded-2xl overflow-hidden border border-white/[0.08] bg-black/40 backdrop-blur-xl shadow-[0_0_60px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.05)]">
                  ...
                  <div>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {project.tech.map((t, i) => (
                        <span key={i} className="px-2.5 py-1 text-[9px] font-bold text-cyan-300 uppercase tracking-wider bg-cyan-400/5 border border-cyan-400/15 rounded-sm font-mono z-20">
                          {t}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6 font-mono text-[10px] pointer-events-none">
                      <div className="border border-white/5 rounded-sm p-2.5 bg-black/20">
                        <div className="text-white/30 uppercase tracking-widest mb-1">Lap Time</div>
                        <div className="text-white font-bold text-sm tracking-widest">{project.lapTime}</div>
                      </div>
                      <div className="border border-white/5 rounded-sm p-2.5 bg-black/20">
                        <div className="text-white/30 uppercase tracking-widest mb-1">Speed Trap</div>
                        <div className="text-white font-bold text-sm tracking-widest">{project.speedTrap}</div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-red-500/30 to-transparent z-10" />
            </SpotlightCard>
          </motion.div>
        )}
        </AnimatePresence>
