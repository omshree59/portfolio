...
const AntiGravityCanvas: React.FC = () => {
  ...
  // 🔥 FIX: Global Mouse Tracking. 
  // This ensures mouse movement is tracked even when hovering over the Chat Box!
  useEffect(() => {
...
// --- Main Wrapper Component for Contact Section ---
interface ParticleEffectWrapperProps {
  children: React.ReactNode;
}

export default function ParticleEffectWrapper({ children }: ParticleEffectWrapperProps) {
  return (
    <div className="relative w-full min-h-screen bg-black overflow-hidden selection:bg-cyan-500 selection:text-white">
      {/* 1. The Canvas Background */}
      <AntiGravityCanvas />
      
      {/* 2. Your Contact Form Content (safely rendered on top via z-index) */}
      <div className="relative z-10 w-full h-full min-h-screen flex items-center justify-center pointer-events-none">
        {/* pointer-events-auto ensures your form can still be clicked */}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-white/30 animate-pulse pointer-events-none z-10">
         <span className="text-[10px] uppercase tracking-[0.2em]">Interact with background</span>
